...
class MultiCursorService {
  ...
  void addCursorsForWord(String word) {
    ...
    while (index < text.length) {
      ...
      _cursors.add(
        TextSelection(baseOffset: index, extentOffset: index + word.length),
      );
      index += word.length;
    }
  ...
  void insertTextAtAllCursors(String text) {
    ...
    for (final cursor in _cursors) {
      if (cursor.start <= newText.length) {
        newText =
            newText.substring(0, cursor.start) +
            text +
  ...
  void backspaceAtAllCursors() {
    ...
    for (final cursor in _cursors) {
      if (cursor.start > 0 && cursor.start <= newText.length) {
        newText =
            newText.substring(0, cursor.start - 1) +
            newText.substring(cursor.end);
      }
    }
  ...
  void selectAtAllCursors(int startOffset, int endOffset) {
    ...
    for (int i = 0; i < _cursors.length; i++) {
      ...
      final newStart = (cursor.start + startOffset).clamp(
        0,
        _controller!.text.length,
      );
      final newEnd = (cursor.start + endOffset).clamp(
        0,
        _controller!.text.length,
      );

      _cursors[i] = TextSelection(baseOffset: newStart, extentOffset: newEnd);
    }
  }
  ...
  void moveAllCursors(int offset) {
    ...
    for (int i = 0; i < _cursors.length; i++) {
      final cursor = _cursors[i];
      final newPosition = (cursor.start + offset).clamp(
        0,
        _controller!.text.length,
      );
  ...
  void duplicateLines() {
    ...
    final sortedLines = linesToDuplicate.toList()
