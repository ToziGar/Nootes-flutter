...
class WeakPasswordException extends AuthenticationException {
  const WeakPasswordException() 
    : super('La contraseña debe tener al menos 6 caracteres', code: 'auth/weak-password');
}
...
class RequiresRecentLoginException extends AuthenticationException {
  const RequiresRecentLoginException() 
    : super('Esta operación requiere autenticación reciente', code: 'auth/requires-recent-login');
}
...
class AccountExistsWithDifferentCredentialException extends AuthenticationException {
  const AccountExistsWithDifferentCredentialException() 
    : super('Ya existe una cuenta con este email usando un proveedor diferente', code: 'auth/account-exists-with-different-credential');
}
...
class CredentialAlreadyInUseException extends AuthenticationException {
  const CredentialAlreadyInUseException() 
    : super('Esta credencial ya está en uso por otra cuenta', code: 'auth/credential-already-in-use');
}
...
class EmailChangeNeedsVerificationException extends AuthenticationException {
  const EmailChangeNeedsVerificationException() 
    : super('El cambio de email requiere verificación', code: 'auth/email-change-needs-verification');
}
