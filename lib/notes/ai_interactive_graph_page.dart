...
class _AIInteractiveGraphPageState extends State<AIInteractiveGraphPage>
  ...
  Offset _calculateGalaxyPosition(int index, int total, ContentAnalysis analysis, Size size) {
    ...
    final angle = baseAngle + (index / total) * 2 * math.pi * 0.2; // Dispersión en el brazo
  ...
  double _calculateSemanticSimilarity(AIGraphNode node1, AIGraphNode node2) {
    ...
    // Similitud por temas
    final sharedThemes = node1.aiAnalysis.themes.toSet()
        .intersection(node2.aiAnalysis.themes.toSet());
    similarity += sharedThemes.length * 0.4;
    
    // Similitud por palabras clave
    final sharedKeywords = node1.aiAnalysis.keywords.toSet()
        .intersection(node2.aiAnalysis.keywords.toSet());
  ...
  String _generateConnectionLabel(AIGraphNode node1, AIGraphNode node2, EdgeType type) {
    switch (type) {
  ...
  List<NodeCluster> _clusterBySemantic(List<AIGraphNode> nodes) {
    ...
    return clusterMap.entries.map((entry) => NodeCluster(
      id: entry.key,
  ...
  List<NodeCluster> _clusterByImportance(List<AIGraphNode> nodes) {
    final highImportance = nodes.where((n) => n.importance > 0.7).map((n) => n.id).toList();
    final mediumImportance = nodes.where((n) => n.importance > 0.4 && n.importance <= 0.7).map((n) => n.id).toList();
    final lowImportance = nodes.where((n) => n.importance <= 0.4).map((n) => n.id).toList();
    
    return [
      if (highImportance.isNotEmpty) NodeCluster(
        id: 'high',
        name: 'Alta Importancia',
        nodeIds: highImportance,
        color: Colors.red,
      ),
      if (mediumImportance.isNotEmpty) NodeCluster(
        id: 'medium',
        name: 'Media Importancia',
        nodeIds: mediumImportance,
        color: Colors.orange,
      ),
      if (lowImportance.isNotEmpty) NodeCluster(
        id: 'low',
  ...
  Widget _buildMainCanvas() {
    return GestureDetector(
      onPanUpdate: _selectedNodeId == null ? (details) {
              ...
              DropdownButton<VisualizationStyle>(
                ...
                items: VisualizationStyle.values.map((style) {
                  return DropdownMenuItem(
                    value: style,
                    child: Text(_getStyleName(style), style: const TextStyle(fontSize: 12)),
                  );
                }).toList(),
                onChanged: (style) {
                  if (style != null) {
          ...
          child: Column(
            ...
            children: [
              ...
              Wrap(
                children: [
                  ...
                  if (node.aiAnalysis.sentiment != 0)
                    _buildInfoChip(
                      node.aiAnalysis.sentiment > 0 ? '😊 Positivo' : '😔 Negativo',
              ...
              if (node.tags.isNotEmpty) ...[
                const SizedBox(height: 8),
                Wrap(
                  children: node.tags.map((tag) => Chip(
              ...
              if (connections.isNotEmpty) ...[
                ...
                ...connections.take(3).map((edge) {
                  ...
                  return ListTile(
                    ...
                    trailing: Text('${(edge.strength * 100).toInt()}%', style: const TextStyle(fontSize: 10)),
                  );
                }),
              ],
            ],
