...
class EdgeEditorDialog extends StatefulWidget {
  ...
  const EdgeEditorDialog({super.key, required this.uid, this.edgeId, this.fromNoteId, this.toNoteId});
...
class _EdgeEditorDialogState extends State<EdgeEditorDialog> {
  ...
  Future<void> _loadEdge() async {
    setState(() => _loading = true);
    try {
      final docs = await FirestoreService.instance.listEdgeDocs(uid: widget.uid);
      final match = docs.firstWhere((d) => d['id'] == widget.edgeId, orElse: () => {});
      if (match.isNotEmpty) {
        ...
        _strength = (match['strength'] is num) ? (match['strength'] as num).toDouble() : _strength;
  ...
  Future<void> _save() async {
    ...
    try {
      if (widget.edgeId == null) {
        final id = await FirestoreService.instance.createEdgeDoc(uid: widget.uid, data: data);
        
        // Sync with legacy links if enabled
        if (_syncWithLegacyLinks) {
          await FirestoreService.instance.addLink(uid: widget.uid, fromNoteId: fromId, toNoteId: toId);
          if (_bidirectional) {
            await FirestoreService.instance.addLink(uid: widget.uid, fromNoteId: toId, toNoteId: fromId);
          }
        }
        
        if (mounted) Navigator.of(context).pop(EdgeEditorResult(edgeId: id, deleted: false));
      } else {
        await FirestoreService.instance.updateEdgeDoc(uid: widget.uid, edgeId: widget.edgeId!, data: data);
        if (mounted) Navigator.of(context).pop(EdgeEditorResult(edgeId: widget.edgeId, deleted: false));
      }
    } catch (e) {
      if (mounted) ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text('Error guardando arista: $e')));
    }
    if (mounted) setState(() => _loading = false);
  }
  ...
  Future<void> _delete() async {
    ...
    setState(() => _loading = true);
    try {
      await FirestoreService.instance.deleteEdgeDoc(uid: widget.uid, edgeId: widget.edgeId!);
      
      // Also remove from legacy links if sync enabled
      if (_syncWithLegacyLinks) {
        final fromId = _selectedFromId ?? _fromCtrl.text.trim();
        final toId = _selectedToId ?? _toCtrl.text.trim();
        await FirestoreService.instance.removeLink(uid: widget.uid, fromNoteId: fromId, toNoteId: toId);
        if (_bidirectional) {
          await FirestoreService.instance.removeLink(uid: widget.uid, fromNoteId: toId, toNoteId: fromId);
        }
      }
      
      if (mounted) Navigator.of(context).pop(EdgeEditorResult(edgeId: widget.edgeId, deleted: true));
    } catch (e) {
  ...
  Widget build(BuildContext context) {
    return AlertDialog(
      ...
      content: _loading ? const SizedBox(height: 80, child: Center(child: CircularProgressIndicator())) : SingleChildScrollView(
        child: Form(
          key: _formKey,
          child: Column(mainAxisSize: MainAxisSize.min, children: [
            ...
            TextFormField(controller: _labelCtrl, decoration: const InputDecoration(labelText: 'Etiqueta (opcional)')),
            const SizedBox(height: 8),
            Row(children: [
              const Text('Fuerza'), 
              Expanded(child: Slider(value: _strength, min: 0.0, max: 1.0, onChanged: (v) => setState(() => _strength = v))),
            ...
            DropdownButtonFormField<String>(
              ...
              items: [
                ...
                const DropdownMenuItem(value: 'thematic', child: Text('📝 Temático')),
                const DropdownMenuItem(value: 'strong', child: Text('💪 Fuerte')),
                const DropdownMenuItem(value: 'weak', child: Text('🔸 Débil')),
              ], 
            ...
            CheckboxListTile(
              ...
              onChanged: (v) => setState(() => _bidirectional = v ?? false), 
              title: const Text('Bidireccional')
            ),
            CheckboxListTile(
              value: _syncWithLegacyLinks, 
              onChanged: (v) => setState(() => _syncWithLegacyLinks = v ?? true), 
              title: const Text('Sincronizar con enlaces legacy'),
              subtitle: const Text('Mantiene compatibilidad con sistema anterior'),
            ),
          ]),
        ),
      ),
      actions: [
        if (widget.edgeId != null) TextButton(onPressed: _delete, child: const Text('Eliminar', style: TextStyle(color: Colors.red))),
