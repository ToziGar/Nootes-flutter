...
class ValidationUtils {
  ...
  static String validateEmail(String email) {
    ...
    final parts = s.split('@');
    if (parts.length != 2) throw const ValidationException('email', 'Formato de email inválido');

    final local = parts[0];
    final domain = parts[1];

    if (local.isEmpty) throw const ValidationException('email', 'Formato de email inválido');
    if (local.startsWith('.') || local.endsWith('.')) throw const ValidationException('email', 'Formato de email inválido');
    if (local.contains('..')) throw const ValidationException('email', 'Formato de email inválido');

    final domainParts = domain.split('.');
    if (domainParts.length < 2) throw const ValidationException('email', 'Formato de email inválido');
    for (final part in domainParts) {
      if (part.isEmpty) throw const ValidationException('email', 'Formato de email inválido');
      if (part.startsWith('-') || part.endsWith('-')) throw const ValidationException('email', 'Formato de email inválido');
    }

    final tld = domainParts.last;
    if (tld.length < 2) throw const ValidationException('email', 'Formato de email inválido');
  ...
  static String validateUsername(String username) {
    ...
    if (trimmed.contains('@') && !trimmed.startsWith('@')) {
      throw const ValidationException('username', 'Username contiene caracteres inválidos');
    }
    
    if (normalizedUsername.length < 3) {
      throw const ValidationException('username', 'Username debe tener al menos 3 caracteres');
    }
    
    if (normalizedUsername.length > 30) {
      throw const ValidationException('username', 'Username no puede tener más de 30 caracteres');
    }
  ...
  static String? validateMessage(String? message) {
    ...
    if (trimmed.length > 500) {
      throw const ValidationException('message', 'Mensaje no puede exceder 500 caracteres');
    }
  ...
  static DateTime? validateExpirationDate(DateTime? expiresAt) {
    ...
    if (expiresAt.isBefore(DateTime.now())) {
      throw const ValidationException('expiresAt', 'Fecha de expiración debe ser en el futuro');
    }
    
    // Máximo 1 año en el futuro
    final maxExpiration = DateTime.now().add(const Duration(days: 365));
    if (expiresAt.isAfter(maxExpiration)) {
      throw const ValidationException('expiresAt', 'Fecha de expiración no puede ser más de 1 año en el futuro');
    }
  ...
  static String validateUserIdentifier(String identifier) {
    ...
    if (trimmed.isEmpty) {
      throw const ValidationException('userIdentifier', 'Identificador de usuario no puede estar vacío');
    }
  ...
  static String validateContent(String content, String fieldName) {
    ...
    if (content.trim().length > 10000) {
      throw ValidationException(fieldName, '$fieldName no puede exceder 10,000 caracteres');
    }
